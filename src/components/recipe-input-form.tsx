...
const ingredientSchema = z.object({
  ...
  name: z.string().min(1, "Ingredient name cannot be empty if added"),
    ...
    <Card className="w-full shadow-none border-0"> {/* Adjusted styling */}
      ...
      <CardContent className="p-0"> {/* Adjusted padding */}
        ...
        <form id="recipe-form" onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
           ...
           {/* Description Field */}
            <div>
                ...
                <Textarea
                    id="description"
                    placeholder="e.g., Quick and easy breakfast..."
          ...
          <div>
            <Label className="block text-sm font-medium mb-2">Ingredients (Optional for Nutrition Estimation)</Label>
            <div className="space-y-3">
              {fields.map((field, index) => (
                <div key={field.id} className="flex items-start space-x-2">
                  <div className="flex-1 space-y-1">
                    {index === 0 && <Label htmlFor={`ingredients.${index}.name`} className="sr-only">Ingredient Name</Label>}
                    <Input
                  ...
                  <div className="w-24 space-y-1"> {/* Slightly narrower width */}
                      {index === 0 && <Label htmlFor={`ingredients.${index}.quantity`} className="sr-only">Quantity (g)</Label>}
                     <Input
